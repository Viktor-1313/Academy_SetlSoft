...
if (closeModal) {
  closeModal.onclick = function() {
    ...
    if (resetLoginBtn) resetLoginBtn.style.display = 'none';
...
if (resetBtn) {
  resetBtn.onclick = function() {
    ...
    if (errorDiv) errorDiv.style.display = 'none';
    const resetCode = document.getElementById('reset-code');
    if (resetCode) resetCode.style.display = 'block';
  };
}
const resetCode = document.getElementById('reset-code');
if (resetCode) {
  resetCode.addEventListener('input', function() {
    ...
    if (this.value.length === 4) {
      if (confirmCodeBtn) confirmCodeBtn.style.display = 'block';
    } else {
      if (confirmCodeBtn) confirmCodeBtn.style.display = 'none';
    }
  });
...
if (confirmCodeBtn) {
  confirmCodeBtn.onclick = function() {
    ...
    if (resetCode.value === '1234') { // Тестовый правильный код
      ...
      const resetLoginBtn = document.getElementById('reset-login-btn');
      if (resetLoginBtn) resetLoginBtn.style.display = 'block';
      const resetError = document.getElementById('reset-error');
      if (resetError) resetError.style.display = 'none';
      ...
      const resetError = document.getElementById('reset-error');
      if (resetError) {
        resetError.textContent = 'Неверный код';
    ...
    try {
      ...
      if (result.success) {
        ...
        if (loginResp.ok && loginResult.success) {
          localStorage.setItem('isLoggedIn', '1');
          localStorage.setItem('userName', (loginResult.profile.firstname || '') + ' ' + (loginResult.profile.lastname || ''));
          localStorage.setItem('userPhoto', loginResult.profile.photo || 'https://avatars.dicebear.com/api/personas/username.svg');
...
// Универсальная логика отображения пользователя в хедере
...
// Пример логики для кнопки "Войти" (тестовая авторизация)
...
// Блокировка перехода для неавторизованных пользователей на главной
